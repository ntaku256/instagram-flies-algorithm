...
class InstaGramFlies:
    ...
    def InitFlies(self):
        ...
        self.vector_strategy = np.array([ "blue" for _ in range(self.n_flies)])
    ...
    def Run(self):
        ...
        for i in range(self.n_iters):
            ...
            # if (not self.centers is None) and ((i - 1) < 2 or (i - 1) == 4 or (i)%10  == 0 ):
            if i == i:
                title = ax.text(0,0,1.61*max(Z),  '%s' % (str(i)), size=20, zorder=1,  color='k') 
                # scatter_best_vector = ax.scatter(self.p_best_vectors.T[0], self.p_best_vectors.T[1],Evaluate(self.p_best_vectors),c="cyan", s=2, alpha=0.5)
                scatter_vector = ax.scatter(self.vectors.T[0], self.vectors.T[1],Evaluate(self.vectors),c=self.vector_strategy, s=3, alpha=0.5)
                ...
                if (not self.centers is None):
                    ...
                    imgs.append([scatter_func,scatter_center,scatter_vector,title])
                else:
        ...
        ani.save("benchmark_function/GIF/insta_files/insta_files_faddist.gif",writer="imagemagick")
        plt.show()
        self.EvaluateLikes()
