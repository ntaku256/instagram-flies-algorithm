...
def roulettMin(table, offset=1e-40):
    total = 0
    for i in range(len(table)):
...
class InstaGramFlies:
    ...
    def Run(self):
        ...
        for i in range(self.n_iters):
            ...
            # # if i >= 0:
            #     title = ax.text(0,0,1.8*1.61*max(Z),  '%s' % (str(i)), size=20, zorder=1,  color='k') 
            ...
            #         scatter_center = ax.scatter(self.centers.T[0], self.centers.T[1],Evaluate(self.centers),c="black", s=5, alpha=0.8)
        ...
        return self.p_best_score[best_arg],self.p_best_vectors[best_arg]
...
if __name__ == "__main__":
    ...
    for j in range(20):
        ...
        for k in range(20):
            master_c3 = 0.1
            for l in range(20):
                print(master_c1,master_c2,master_c3)
